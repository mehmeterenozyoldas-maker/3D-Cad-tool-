...
export type FurnitureType = 'Chair' | 'Table' | 'Stool' | 'Bench' | 'Shelf' | 'Vase' | 'Recliner' | 'Lamp' | 'Mobius' | 'Hyperbolic' | 'Klein' | 'FractalTree';
export type FurniturePattern = 'Linear' | 'Voronoi' | 'Gyroid' | 'Triangular' | 'SchwarzD' | 'Octet' | 'Sponge';
...
function isInsideFurniture(type: FurnitureType, x: number, y: number, z: number, params: FurnitureParams): boolean {
  ...
  if (type === 'Hyperbolic') {
    // Saddle shape: y = x^2 - z^2
    const nx = x / (width/2);
    const nz = z / (depth/2);
    const saddleY = (height/2) + (nx*nx - nz*nz) * (height/4);
    const thickness = height * 0.15;
    return Math.abs(y - saddleY) < thickness && (x*x + z*z) <= (width/2)*(width/2);
  }
...
function generateVolumeVoronoi(nodes: Vector3[], edges: [number, number][], params: FurnitureParams, type: FurnitureType) {
  ...
  while(points.length < count && attempts < count * 10) {
    ...
    if (isInsideFurniture(type, x, y, z, params)) {
      const id = addNode(nodes, x, y, z);
      points.push({ id, vec: new Vector3(x, y, z) });
    }
  }
  ...
  // Lloyd's Relaxation (Approximation) to create true Voronoi-like uniform cells
  for (let iter = 0; iter < 3; iter++) {
    const centroids = points.map(() => new Vector3());
    const counts = points.map(() => 0);

    for(let i=0; i<points.length; i++) {
      for(let j=i+1; j<points.length; j++) {
        if (points[i].vec.distanceTo(points[j].vec) < threshold) {
          centroids[i].add(points[j].vec); counts[i]++;
          centroids[j].add(points[i].vec); counts[j]++;
        }
      }
    }
    ...
    for(let i=0; i<points.length; i++) {
      if (counts[i] > 0) {
        centroids[i].divideScalar(counts[i]);
        // Move halfway to centroid for stability, keeping it organic
        points[i].vec.lerp(centroids[i], 0.5);
      }
    }
  }

  // Update nodes array with relaxed positions
  points.forEach(p => nodes[p.id].copy(p.vec));
...
function generateVolumeSchwarzD(nodes: Vector3[], edges: [number, number][], params: FurnitureParams, type: FurnitureType) {
  const { width, height, depth } = params;
  const res = 0.08; 
  const scale = 8.0; 
  ...
  for (let x = 0; x < nx; x++) {
    for (let y = 0; y < ny; y++) {
      for (let z = 0; z < nz; z++) {
        ...
        const gy = wy * scale;
        const gz = wz * scale;
        const val = Math.sin(gx)*Math.sin(gy)*Math.sin(gz) + 
                    Math.sin(gx)*Math.cos(gy)*Math.cos(gz) + 
                    Math.cos(gx)*Math.sin(gy)*Math.cos(gz) + 
                    Math.cos(gx)*Math.cos(gy)*Math.sin(gz);
  ...
  nodeMap.forEach((realIdx, gridId) => {
    ...
    neighbors.forEach(([nx_coord, ny_coord, nz_coord]) => {
      const nid = idx(nx_coord, ny_coord, nz_coord);
      if (nodeMap.has(nid)) link(edges, realIdx, nodeMap.get(nid)!);
    });
  });
}

function generateVolumeSponge(nodes: Vector3[], edges: [number, number][], params: FurnitureParams, type: FurnitureType) {
  const { width, height, depth } = params;
  const res = 0.08; 
  const scale = 5.0; 
  ...
  for (let x = 0; x < nx; x++) {
    for (let y = 0; y < ny; y++) {
      for (let z = 0; z < nz; z++) {
        ...
        const n1 = noise3(wx*scale, wy*scale, wz*scale);
        const n2 = noise3(wx*scale*2.1 + 15, wy*scale*2.1 + 15, wz*scale*2.1 + 15) * 0.5;
...
function generateVolumeOctet(nodes: Vector3[], edges: [number, number][], params: FurnitureParams, type: FurnitureType) {
  const { width, height, depth } = params;
  const cellSize = 0.12; 
  
  const nx = Math.ceil(width / cellSize) + 2;
  const ny = Math.ceil(height / cellSize) + 2;
  const nz = Math.ceil(depth / cellSize) + 2;
  
  const idx = (x: number, y: number, z: number) => x + y * nx + z * nx * ny;
  const nodeMap = new Map<number, number>();
  
  const offsetX = -width/2 - cellSize;
  const offsetY = -cellSize;
  const offsetZ = -depth/2 - cellSize;
  
  // FCC lattice: x+y+z is even
  for (let x = 0; x <= nx; x++) {
      for (let y = 0; y <= ny; y++) {
          for (let z = 0; z <= nz; z++) {
  ...
  // Connect FCC neighbors (distance sqrt(2))
  const dirs = [
  ...
  nodeMap.forEach((realIdx, gridId) => {
      ...
      dirs.forEach(([dx, dy, dz]) => {
          // Only connect forward to avoid double edges
...
function generateKlein(nodes: Vector3[], edges: [number, number][], params: FurnitureParams) {
    const { width, height } = params;
    const segmentsU = 40;
    const segmentsV = 16;
    const grid: number[][] = [];
    ...
    for(let i=0; i<=segmentsU; i++) {
        ...
        for(let j=0; j<=segmentsV; j++) {
            ...
            row.push(addNode(nodes, x * 1.5, z * height * 0.4 + height/2, y * 1.5));
        }
        grid.push(row);
    }
    ...
    for(let i=0; i<segmentsU; i++) {
        for(let j=0; j<segmentsV; j++) {
            link(edges, grid[i][j], grid[i+1][j]);
            link(edges, grid[i][j], grid[i][j+1]);
        }
    }
}

function generateFractalTree(nodes: Vector3[], edges: [number, number][], params: FurnitureParams) {
    const { height } = params;
    const maxDepth = 5;
    
    function branch(startIdx: number, startVec: Vector3, dirVec: Vector3, len: number, depth: number) {
        const endVec = startVec.clone().add(dirVec.clone().multiplyScalar(len));
        const endIdx = addNode(nodes, endVec.x, endVec.y, endVec.z);
        link(edges, startIdx, endIdx);
        
        if (depth > 0) {
            const branches = 3;
            for(let i=0; i<branches; i++) {
                const spread = 0.6 + Math.random()*0.4;
                const twist = (i / branches) * Math.PI * 2 + Math.random()*0.5;
                
                const q1 = new Quaternion().setFromAxisAngle(new Vector3(1,0,0), spread);
                const q2 = new Quaternion().setFromAxisAngle(dirVec, twist);
                const newDir = dirVec.clone().applyQuaternion(q1).applyQuaternion(q2).normalize();
                
                branch(endIdx, endVec, newDir, len * 0.7, depth - 1);
            }
        }
    }
    const rootIdx = addNode(nodes, 0, 0, 0);
    branch(rootIdx, new Vector3(0,0,0), new Vector3(0,1,0), height * 0.35, maxDepth);
}
...
export function generateFurniture(type: FurnitureType, params: FurnitureParams): NetworkData {
  ...
  const isVolumePattern = params.pattern === 'Gyroid' || params.pattern === 'Voronoi' || params.pattern === 'Triangular' || params.pattern === 'SchwarzD' || params.pattern === 'Octet' || params.pattern === 'Sponge';
  
  if (isVolumePattern && type !== 'Klein' && type !== 'FractalTree' && type !== 'Mobius' && type !== 'Lamp' && type !== 'Recliner') {
      ...
      } else if (params.pattern === 'SchwarzD') {
          generateVolumeSchwarzD(nodes, edges, params, type);
      } else if (params.pattern === 'Octet') {
          generateVolumeOctet(nodes, edges, params, type);
      } else if (params.pattern === 'Sponge') {
          generateVolumeSponge(nodes, edges, params, type);
      } else {
  ...
  else if (type === 'Klein') generateKlein(nodes, edges, params);
  else if (type === 'FractalTree') generateFractalTree(nodes, edges, params);
  else if (type === 'Hyperbolic') {
      // If hyperbolic is selected but pattern is linear, fallback to octet to show the shape
      generateVolumeOctet(nodes, edges, params, type);
  }
  else if (type === 'Bench') {
      // Inline Linear Bench
